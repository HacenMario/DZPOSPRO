  ...
  const rows = state.items.map((s, i) => {
    ...
    return `
      <tr>
        ...
        <td class="cell-strong" data-label="${t('invoiceNumber', 'Invoice #')}">
...
/* Fetch ALL invoices matching the current filters (paged loop, cap 50 pages).
 * Used by stats + CSV export so they cover the whole filtered set, not only
...
async function fetchAllInvoices() {
  ...
  for (let page = 1; page <= MAX_PAGES; page++) {
    ...
    const items = r.data || r.sales || [];
    all.push(...items);
    if (total == null && r.total != null) total = r.total;
    if (!items.length) break;
    if (total != null && all.length >= total) break;
    if (r.totalPages != null && page >= r.totalPages) break;
  }
  return all;
}

async function computeStats() {
  // Stats must cover ALL filtered invoices, not only the current page
  let items = [];
  try { items = await fetchAllInvoices(); } catch (_) { items = []; }
  if (!items.length) items = state.items; // graceful fallback: current page
  const todayStr = new Date().toDateString();
  const total = state.pagination ? state.pagination.total : items.length;
...
async function exportCsv() {
  // Export covers ALL filtered invoices (not only the current page)
  let items = [];
  try { items = await fetchAllInvoices(); } catch (_) { items = []; }
  if (!items.length) items = state.items;
  if (!items.length) {
...
async function downloadInvoicePdf(sale) {
  try {
    ...
    // jsPDF core fonts cannot render Arabic (garbled output). When Arabic text
    // is present, rasterize the rendered receipt via html2canvas, or fall back
    // to the browser print dialog (choose "Save as PDF").
    const hasArabic = /[\u0600-\u06FF]/.test(
      [store.storeName || '', customerNameStr].concat(items.map(it => it.name || '')).join(' ')
    );
    if (hasArabic) {
      const el = document.querySelector('#invoiceDetailModal .receipt-sheet');
      if (el && typeof html2canvas !== 'undefined') {
        const doc2 = new window.jspdf.jsPDF('p', 'mm', 'a4');
        doc2.html(el, {
          callback: (d) => d.save('invoice-' + invoiceNo + '.pdf'),
